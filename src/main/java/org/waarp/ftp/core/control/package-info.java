 ...
 * It first creates the default startup command (ConnectionCommand), then it answers it is ok to accept identification
 * (which is implied by ConnectionCommand).
 ...
 * allows the usage of the current command (for instance, no transfer command is allowed if the authentication is not
 * finished).
