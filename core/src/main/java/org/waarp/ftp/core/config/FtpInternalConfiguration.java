...
public class FtpInternalConfiguration {
  ...
  public FtpInternalConfiguration(FtpConfiguration configuration) {
    ...
    execBoss =
        new NioEventLoopGroup(configuration.getSERVER_THREAD(),
                              new WaarpThreadFactory("CommandBoss", false));
    ...
    execPassiveDataBoss =
        new NioEventLoopGroup(configuration.getSERVER_THREAD() * 2,
                              new WaarpThreadFactory(
                                  "PassiveDataBoss"));
    execDataWorker =
        new NioEventLoopGroup(configuration.getCLIENT_THREAD() * 2,
  ...
  /**
   * Try to unbind (closing the parent channel) the Passive Channel listening to
   * the specified local address if the
   * last one. It returns only when the underlying parent channel is closed if
   * this was the last session that wants to
   * open on this local address.
   *
   * @param address
   */
