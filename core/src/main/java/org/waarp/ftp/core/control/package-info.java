 ...
 * answers it is ok to accept identification
 ...
 * currently running except
 * if is a special command (like QUIT or ABORT).
 * <li>Checking if the command is legal from workflow</li> that is to say the
 * previous command
 * allows the usage of the current command (for instance, no transfer command is
 * allowed if the authentication is not
 * finished).
