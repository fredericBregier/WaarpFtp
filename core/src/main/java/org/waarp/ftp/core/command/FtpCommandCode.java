 ...
 * comment says the object of the command and
 * the kind of returned codes that could follow this command.<br>
 * <br>
 * Command structure:<br> Main class<br> Previous Valid Command (null means all
 * are valid)<br> Next Valid Commands (none
...
public enum FtpCommandCode {
   ...
   * identification is that which is required by
   * the server for access to its file system. This command will normally be the
   * first command transmitted by the user
   * after the control connections are made (some servers may require this).
   * Additional identification information in
   * the form of a password and/or an account command may also be required by
   * some servers. Servers may allow a new
   * USER command to be entered at any point in order to change the access
   * control and/or accounting information. This
   * has the effect of flushing any user, password, and account information
   * already supplied and beginning the login
   * sequence again. All transfer parameters are unchanged and any file transfer
   * in progress is completed under the
   ...
   * command must be immediately preceded
   * by the user name command, and, for some sites, completes the user's
   * identification for access control. Since
   * password information is quite sensitive, it is desirable in general to
   * "mask" it or suppress typeout. It appears
   * that the server has no foolproof way to achieve this. It is therefore the
   * responsibility of the user-FTP process
   ...
   * account information is required for
   * login, the response to a successful PASSword command is reply code 332. On
   * the other hand, if account information
   * is NOT required for login, the reply to a successful PASSword command is
   * 230; and if the account information is
   * needed for a command issued later in the dialogue, the server should return
   * a 332 or 532 reply depending on
   * whether it stores (pending receipt of the ACCounT command) or discards the
   ...
   * for file storage or retrieval without
   * altering his login or accounting information. Transfer parameters are
   * similarly unchanged. The argument is a
   ...
   * The reply codes shall be identical to the reply codes of CWD. See Appendix
   * II for further details.<br>
   * <p>
   * <p>
   ...
   * argument is a pathname specifying a
   ...
   * progress to be completed. All parameters are reset to the default settings
   * and the control connection is left
   * open. This is identical to the state in which a user finds himself
   * immediately after the control connection is
   ...
   * If file transfer is in progress, the connection will remain open for result
   * response and the server will then
   * close it. If the user-process is transferring files for several USERs but
   * does not wish to close and then reopen
   * connections for each, then the REIN command should be used instead of
   * QUIT.<br>
   * <br>
   * <p>
   * An unexpected close on the control connection will cause the server to take
   * the effective action of an abort
   ...
   * data connection. There are defaults for
   * both the user and server data ports, and under normal circumstances this
   * command and its reply are not needed. If
   * this command is used, the argument is the concatenation of a 32-bit
   * internet host address and a 16-bit TCP port
   * address. This address information is broken into 8-bit fields and the value
   * of each field is transmitted as a
   * decimal number (in character string representation). The fields are
   * separated by commas. A port command would
   * be:<br>
   * <br>
   *
   * <pre>
   ...
   * not its default data port) and to wait
   ...
   * single Telnet character, as is the
   * second Format parameter for ASCII and EBCDIC; the second parameter for
   * local byte is a decimal integer to
   * indicate Bytesize. The parameters are separated by a
   * <code>&lt;SP&gt;</code> (Space, ASCII code 32).<br>
   * <br>
   * <p>
   ...
   * is changed, and later just the first
   ...
   * modes described in the Section on
   ...
   * specified in the pathname, to the server- or
   * user-DTP at the other end of the data connection. The status and contents
   * of the file at the server site shall be
   * unaffected.<br>
   * <p>
   * 125, 150<br> (110)<br> 226, 250<br> 425, 426, 451<br> 450, 550<br> 500,
   * 501, 421, 530<br>
   */
   ...
   * as a file at the server site. If the file specified in the pathname exists
   * at the server site, then its contents
   * shall be replaced by the data being transferred. A new file is created at
   * the server site if the file specified
   * in the pathname does not already exist.<br>
   * <p>
   * <p>
   * 125, 150<br> (110)<br> 226, 250<br> 425, 426, 451, 551, 552<br> 532, 450,
   * 452, 553<br> 500, 501, 421, 530<br>
   */
   ...
   * created in the current directory under a
   * name unique to that directory. The 250 Transfer Started response must
   ...
   * This command causes the server-DTP to accept the data transferred via the
   * data connection and to store the data
   * in a file at the server site. If the file specified in the pathname exists
   * at the server site, then the data
   * shall be appended to that file; otherwise the file specified in the
   * pathname shall be created at the server
   * site.<br>
   * <p>
   * <p>
   * 125, 150<br> (110)<br> 226, 250<br> 425, 426, 451, 551, 552<br> 532, 450,
   ...
   * to accommodate the new file to be
   * transferred. The argument shall be a decimal integer representing the
   * number of bytes (using the logical byte
   * size) of storage to be reserved for the file. For files sent with record or
   * page structure a maximum record or
   * page size (in logical bytes) might also be necessary; this is indicated by
   * a decimal integer in a second argument
   * field of the command. This second argument is optional, but when present
   * should be separated from the first by
   * the three Telnet characters <code>&lt;SP&gt;</code> R
   * <code>&lt;SP&gt;</code>. This command shall be followed by
   * a STORe or APPEnd command. The ALLO command should be treated as a NOOP (no
   * operation) by those servers which do
   * not require that the maximum size of the file be declared beforehand, and
   * those servers interested in only the
   * maximum record or page size should accept a dummy value in the first
   * argument and ignore it.<br>
   * <p>
   * <p>
   * 125, 150<br> 226, 250<br> 425, 426, 451<br> 450<br> 500, 501, 502, 421,
   * 530<br>
   */
   ...
   * to be restarted. This command does not
   * cause file transfer but skips over the file to the specified data
   * checkpoint. This command shall be immediately
   * followed by the appropriate FTP service command which shall cause file
   * transfer to resume.<br>
   * <br>
   * The current implementation allows restart only on Stream since others would
   * imply to store those informations
   * somewhere (how?).<br>
   * <br>
   * However, it could be changed if necessary by modifying the {@link FtpFile}
   ...
   * This command must be immediately
   * followed by a "rename to" RNTO command specifying the new file
   * pathname.<br>
   * <p>
   * <p>
   ...
   * immediately preceding "rename from" RNFR
   ...
   * any associated transfer of data. The
   * abort command may require "special action", as discussed in the Section on
   * FTP Commands, to force recognition by
   * the server. No action is to be taken if the previous command has been
   * completed (including data transfer). The
   ...
   * closes the data connection, returning a 426
   * reply to indicate that the service request terminated abnormally. The
   * server then sends a 226 reply, indicating
   ...
   * server site. If an extra level of
   * protection is desired (such as the query, "Do you really wish to delete?"),
   * it should be provided by the user-FTP
   * process.<br>
   * <p>
   * <p>
   ...
   * as a directory (if the pathname is
   * absolute) or as a subdirectory of the current working directory (if the
   ...
   * directory or other system-specific file group descriptor; a null argument
   * implies the current directory. The
   * server will return a stream of names of files and no other information. The
   * data will be transferred in ASCII or
   * EBCDIC type over the data connection as valid pathname strings separated by
   * <code>&lt;CRLF&gt;</code> or
   * <code>&lt;NL&gt;</code>. (Again the user must ensure that the TYPE is
   * correct.) This command is intended to
   * return information that can be used by a program to further process the
   * files automatically. For example, in the
   ...
   * system that are essential to file transfer
   ...
   * server. The reply shall have as its first
   * word one of the system names listed in the current version of the Assigned
   * Numbers document.<br>
   * <br>
   ...
   * connection in the form of a reply. The
   * command may be sent during a file transfer (along with the Telnet IP and
   * Synch signals--see the Section on FTP
   * Commands) in which case the server will respond with the status of the
   * operation in progress, or it may be sent
   * between file transfers. In the latter case, the command may have an
   * argument field. If the argument is a
   * pathname, the command is analogous to the "list" command except that data
   * shall be transferred over the control
   * connection. If a partial pathname is given, the server may respond with a
   * list of file names or attributes
   * associated with that specification. If no argument is given, the server
   * should return general status information
   * about the server FTP process. This should include current values of all
   * transfer parameters and the status of
   * connections.<br>
   * <p>
   * <p>
   ...
   * its implementation status over the
   * control connection to the user. The command may take an argument (e.g., any
   * command name) and return more
   * specific information as a response. The reply is type 211 or 214. It is
   * suggested that HELP be allowed before
   * entering a USER command. The server may use this reply to specify
   * site-dependent parameters, e.g., in response to
   * HELP SITE.<br>
   * <p>
   * <p>
   ...
   * It specifies no action other than
  ...
  // XXX RFC3659
  /**
   * The FTP command, MODIFICATION TIME (MDTM), can be used to determine when a
   * file in the server NVFS was last
   * modified.<br>
   * <br>
   * <p>
   * The "pathname" specifies an object in the NVFS that may be the object of a
   * RETR command. Attempts to query the
   * modification time of files that exist but are unable to be retrieved may
   * generate an error- response, or can
   * result in a positive response carrying a time-val with an unspecified
   * value, the choice being made by the
   * server-PI.<br>
   * <br>
   * <p>
   * The server-PI will respond to the MDTM command with a 213 reply giving the
   * last modification time of the file
   * whose pathname was supplied, or a 550 reply if the file does not exist, the
   * modification time is unavailable, or
   ...
   * of a file from the server-FTP process.
   * This is the exact number of octets (8 bit bytes) that would be transmitted
   * over the data connection should that
   * file be transmitted. This value will change depending on the current
   * STRUcture, MODE, and TYPE of the data
   * connection or of a data connection that would be created were one created
   * now. Thus, the result of the SIZE
   ...
   * current transfer structure, mode, and type. This command is normally used
   * in conjunction with the RESTART (REST)
   * command when STORing a file to a remote server in STREAM mode, to determine
   * the restart point. The server-PI
   * might need to read the partially transferred file, do any appropriate
   * conversion, and count the number of octets
   * that would be generated when sending the file in order to correctly respond
   * to this command. Estimates of the
   * file transfer size MUST NOT be returned; only precise information is
   * acceptable.<br>
   * <p>
   * <p>
   * 213<br> 500, 501, 550<br>
   ...
   * process. This command differs from the LIST command in that the format of
   * the replies is strictly defined
   * although extensible.<br>
   * <br>
   * <p>
   * MLSD lists the contents of a directory if a directory is named, otherwise a
   * 501 reply is returned. If no object
   * is named, the current directory is assumed. That will cause MLSD to list
   ...
   * <br>
   * <p>
   * MLST provides data about exactly the object named on its command line, and
   * no others. If no object is named, the
   * current directory is assumed. That will cause MLST to send a one-line
   * response, describing the current directory
   * itself.<br>
   * <p>
   * 125, 150<br> 226, 250<br> 425, 426, 451<br> 450<br> 500, 501, 502, 421,
   * 530<br>
   ...
   * respond to the FEAT command with a 500 or
   * 502 reply. This is simply the normal "unrecognized command" reply that any
   * unknown command would elicit. Errors
   * in the command syntax, such as giving parameters, will result in a 501
   * reply.<br>
   * <br>
   * <p>
   * Server-FTP processes that recognize the FEAT command, but implement no
   * extended features, and therefore have
   * nothing to report, SHOULD respond with the "no-features" 211 reply.
   * However, as this case is practically
   * indistinguishable from a server-FTP that does not recognize the FEAT
   * command, a 500 or 502 reply MAY also be
   * used. The "no-features" reply MUST NOT use the multi-line response format,
   * exactly one response line is required
   * and permitted.<br>
   * <br>
   * <p>
   * Replies to the FEAT command MUST comply with the following syntax. Text on
   * the first line of the reply is free
   * form, and not interpreted, and has no practical use, as this text is not
   * expected to be revealed to end users.
   * The syntax of other reply lines is precisely defined, and if present, MUST
   * be exactly as specified.<br>
   * <br>
   *
   * <pre>
   ...
   * space. That space is not optional, nor
   * does it indicate general white space. This space guarantees that the
   * feature line can never be misinterpreted as
   * the end of the feature-listing, but is required even where there is no
   * possibility of ambiguity.<br>
   * <br>
   * <p>
   * Each extension supported must be listed on a separate line to facilitate
   * the possible inclusion of parameters
   * supported by each extension command. The feature-label to be used in the
   * response to the FEAT command will be
   * specified as each new feature is added to the FTP command set. Often it
   * will be the name of a new command added,
   * however this is not required. In fact it is not required that a new feature
   * actually add a new command. Any
   * parameters included are to be specified with the definition of the command
   * concerned. That specification shall
   ...
   * the definitions of specific labels and
   * parameters specify the precise interpretation, and it is to be expected
   * that those definitions will usually
   * specify the label and parameters in a case independent manner. Where this
   * is done, implementations are
   * recommended to use upper case letters when transmitting the feature
   * response.<br>
   * <br>
   * <p>
   * The FEAT command itself is not included in the list of features supported,
  ...
  /**
   * The OPTS (options) command allows a user-PI to specify the desired behavior
   * of a server-FTP process when another
   * FTP command (the target command) is later issued. The exact behavior, and
   * syntax, will vary with the target
   * command indicated, and will be specified with the definition of that
   * command. Where no OPTS behavior is defined
   * for a particular command there are no options available for that
   * command.<br>
   * <p>
   * <p>
   ...
   * transport addresses. The format of EPRT
   * is:<br>
   * <br>
   *
   * <pre>
   ...
   * <p>
   * The <code>&lt;net-prt&gt;</code> argument MUST be an address family number
   * defined by IANA in the latest Assigned
   * Numbers RFC (RFC 1700 [RP94] as of the writing of this document). This
   * number indicates the protocol to be used
   * (and, implicitly, the address length). This document will use two of
   * address family numbers from [RP94] as
   ...
   * representation of the network address. For the
   * two address families specified above (AF Number 1 and 2), addresses MUST
   * be
   ...
   * representation of the number of the TCP port on
  ...
  /**
   * The EPSV command requests that a server listen on a data port and wait for
   * a connection. The EPSV command takes
   * an optional argument. The response to this command includes only the TCP
   * port number of the listening connection.
   * The format of the response, however, is similar to the argument of the
   * EPRT
   * command. This allows the same parsing
   * routines to be used for both commands. In addition, the format leaves a
   * place holder for the network protocol
   * and/or network address, which may be needed in the EPSV response in the
   * future. The response code for entering
   ...
   *
   * <br>
   * The portion of the string enclosed in parentheses MUST be the exact string
   * needed by the EPRT command to open the
   * data connection, as specified above.<br>
   * <br>
   * <p>
   * The first two fields contained in the parenthesis MUST be blank. The third
   * field MUST be the string
   * representation of the TCP port number on which the server is listening for
   ...
   * be
   * the same network address used for the
   ...
   *
   * <br>
   * The standard negative error codes 500 and 501 are sufficient to handle all
   * errors involving the EPSV command
   ...
   * connection based on the protocol used for the control connection. However,
   * in the case of proxy FTP, this
   * protocol might not be appropriate for communication between the two
   * servers. Therefore, the client needs to be
   * able to request a specific protocol. If the server returns a protocol that
   * is not supported by the host that will
   * be connecting to the port, the client MUST issue an ABOR (abort) command
   * to
   * allow the server to close down the
   * listening connection. The client can then send an EPSV command requesting
   * the use of a specific network protocol,
   * as follows:<br>
   * <br>
   *
   * <pre>
   ...
   * protocol. If not, the server MUST return
   ...
   * Network Address Translators that the EPRT
   * command (as well as other data commands) will no longer be used. An
   * example
   * of this command follows:<br>
   * <br>
   *
   * <pre>
   * EPSV &lt; space &gt; ALL
   ...
   * (i.e., EPRT, PORT, PASV, et al.). This use of the EPSV command is further
   * explained in section 4.<br>
   * <p>
   * <p>
  ...
  /**
   * Compute CRC on pathname given as argument. Return on control network as 250
   * "CRC" is CRC of file "pathname"<br>
   * <p>
   * 250<br> 500, 501, 502, 504, 421, 530<br>
   */
  XCRC(org.waarp.ftp.core.command.extension.XCRC.class, null),
  /**
   * Compute MD5 on pathname given as argument. Return on control network as 250
   * "MD5" is MD5 of file "pathname"<br>
   * <p>
   * 250<br> 500, 501, 502, 504, 421, 530<br>
   */
  ...
  /**
   * Shutdown command (internal password protected command).<br> Shutdown the
   * FTP service<br>
   */
  INTERNALSHUTDOWN(
      ...
      null),
  /**
   * Change the Limit of the global bandwidth.<br> No argument reset to default,
   * 1 argument change both write and read
   * to same value, 2 arguments stand for write then read limit.<br> Limit is
   * written in byte/s. Example:
   * "LIMITBANDWIDTH 104857600 104857600" stands for 100MB/s limitation
   * globaly.<br> -1 means no limit
   */
  LIMITBANDWIDTH(
   ...
   * Data)<br> 234*<br> 502, 504, 534*, 431*
   * 500, 501, 421<br>
   * <br>
  ...
  AUTH(org.waarp.ftp.core.command.rfc4217.AUTH.class, null,
       ...
       org.waarp.ftp.core.command.access.USER.class),
  /**
   * Security Association Setup<br> CCC (Control SSL Off)<br> 200<br> 500, 533*,
   * 534*
   */
  CCC(org.waarp.ftp.core.command.rfc4217.CCC.class, null),
  /**
   * Data protection negotiation commands<br> PROT P (Data)<br> PROT C (Data SSL
   * Off)<br> 200<br> 504, 536*, 503,
   * 534*, 431* 500, 501, 421, 530
   */
  PROT(org.waarp.ftp.core.command.rfc4217.PROT.class, null),
  /**
   * Data protection negotiation commands<br> PBSZ 0<br> 200<br> 503, 500, 501,
   * 421, 530
   */
