...
/**
 * First CODEC :<br> - encode : takes a {@link DataBlock} and transforms it to a
 * ByteBuf<br> - decode : takes a ByteBuf
 * and transforms it to a {@link DataBlock}<br> STREAM and BLOCK mode are
 * implemented. COMPRESSED mode is not
 * implemented.
 *
...
public class FtpDataModeCodec extends ByteToMessageCodec<DataBlock> {
  ...
  protected ByteBuf encode(DataBlock msg)
    ...
    if (mode == TransferMode.STREAM) {
      ...
      ByteBuf newbuf = ByteBufAllocator.DEFAULT
