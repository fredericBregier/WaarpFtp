...
public abstract class BusinessHandler {
  ...
  protected String getDefaultFeatMessage() {
    ...
    builder.append(FtpCommandCode.MDTM.name()).append('\n')
           ...
           .append('\n')
           .append(FtpCommandCode.XMD5.name()).append(" \"filename\"")
           .append('\n')
   ...
   * @param line
   *
   * @return the AbstractCommand to execute if it is a Specialized Command, else
   *     Null
   */
  ...
  protected String getMLSxOptsMessage(String[] args) {
    ...
    FilesystemBasedOptsMLSxImpl optsMLSx =
        (FilesystemBasedOptsMLSxImpl) getFtpSession()
            .getDir().getOptsMLSx();
   ...
   * operation on
   * FtpAuth in order to relax constraints on user authentication. It will be
   * called however at least when the session
   * will be clean just after this call.</I>
   */
   ...
   * answering OK or not on connection,
  ...
  /**
   * Run when an exception is get before the channel is closed. This must set a
   * correct answer.
   *
   * @param e
   */
   ...
   * the command. If an exception is raised,
   * the reply is immediate and no action taken.
   *
   * @throws CommandAbstractException
   */
  ...
  /**
   * This method is called for every received message after the execution of the
   * command but before the final reply to
   * the client. If an exception is raised, the reply is immediate. This is the
   * last call before finishing the
   * command.
   *
   * @throws CommandAbstractException
   */
   ...
   * after). This must set a correct answer and
   * a correct code of reply. If the code of reply is 421, then the channel will
   * be closed after this call. This is
   * the last call before finishing the command.
   *
   * @param e
   */
   ...
   * Note that this is called only for a
   * Transfer Request (or LIST) but called before afterRunCommandXX is called
   * (Ok or Ko).
   *
   * @param transfer
   *
