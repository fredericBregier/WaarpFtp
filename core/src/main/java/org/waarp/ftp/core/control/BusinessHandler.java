...
public abstract class BusinessHandler {
     ...
     * FtpAuth in order to relax constraints on user authentication. It will be called however at least when the session
    ...
    /**
     * This method is called for every received message before the execution of the command. If an exception is raised,
    ...
    /**
     * This method is called for every received message after the execution of the command but before the final reply to
     * the client. If an exception is raised, the reply is immediate. This is the last call before finishing the
     * command.
     *
     * @throws CommandAbstractException
     */
    ...
    /**
     * Run when a FTP exception is catch (the channel is not necessary closed after). This must set a correct answer and
     * a correct code of reply. If the code of reply is 421, then the channel will be closed after this call. This is
    ...
    /**
     * Run when a transfer is finished (eventually in error) but before answering. Note that this is called only for a
     * Transfer Request (or LIST) but called before afterRunCommandXX is called (Ok or Ko).
     *
     * @param transfer
     *
     * @throws CommandAbstractException
