...
public abstract class FtpConfiguration {
   ...
   * client for Executor in the Pipeline for
   * Business logic. Server will change this value on startup if not set.
   * Default 0 means in proportion of real core
   * number.
   */
   ...
   * monitors with values divided by 10
   * (channel = global / 10)
   *
   * @param writeLimit
   * @param readLimit
  ...
  public void changeNetworkLimit(long writeLimit, long readLimit) {
    ...
    FtpGlobalTrafficShapingHandler fgts =
